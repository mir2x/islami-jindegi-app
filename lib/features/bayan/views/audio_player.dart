...
import '../providers/bayan_audio_player.dart';

/// Bayan-local audio player — plays a flat `audioUrl` (from the .NET API)
/// directly, unlike the shared `AudioPlayerWidget` which expects a
/// JSON:API-shaped Shrine attachment map. See bayan_audio_player.dart for why
/// this couldn't just reuse the shared widget/provider.
class BayanAudioPlayer extends ConsumerWidget {
  const BayanAudioPlayer({
    super.key,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final params = (bayanId: bayanId, title: title, audioUrl: audioUrl);
    var audioPlayer = ref.watch(bayanAudioPlayerProvider(params));
    ...
    return audioPlayer.when(
          ...
          child: Container(
            ...
            child: isConnectivityError
                ...
                : IconButton(
                    ...
                    iconSize: 40,
                    onPressed: () {
                      ref.invalidate(bayanAudioPlayerProvider(params));
                    },
                  ),
          ),
        );
      ...
      data: (updatedPlayer) {
        return _StatefulBayanAudioPlayer(player: updatedPlayer);
      },
    );
  }
}

class _StatefulBayanAudioPlayer extends ConsumerStatefulWidget {
  const _StatefulBayanAudioPlayer({required this.player});

  final AudioPlayer player;

  @override
  ConsumerState<_StatefulBayanAudioPlayer> createState() =>
      _StatefulBayanAudioPlayerState();
}

class _StatefulBayanAudioPlayerState
    extends ConsumerState<_StatefulBayanAudioPlayer> {
    ...
    _playbackEventSubscription = widget.player.playbackEventStream.listen(
      (event) {
        if (isPlaying && (event.processingState == ProcessingState.idle)) {
          EasyDebounce.debounce('bayan-player-idle', const Duration(seconds: 10), () {
            if (isPlaying && isIdle) {
              widget.player.stop();
            }
          });
