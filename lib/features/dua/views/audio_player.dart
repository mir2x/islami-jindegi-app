...
import '../providers/dua_audio_player.dart';

/// Dua-local audio player — plays a flat `audioUrl` (from the .NET API)
/// directly, unlike the shared `AudioPlayerWidget` which expects a
/// JSON:API-shaped Shrine attachment map. See dua_audio_player.dart for why
/// this couldn't just reuse the shared widget/provider.
class DuaAudioPlayer extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final params = (duaId: duaId, title: title, audioUrl: audioUrl);
    var audioPlayer = ref.watch(duaAudioPlayerProvider(params));
    ...
    return audioPlayer.when(
          ...
          child: Container(
            ...
            child: isConnectivityError
                ...
                : IconButton(
                    ...
                    iconSize: 40,
                    onPressed: () {
                      ref.invalidate(duaAudioPlayerProvider(params));
                    },
                  ),
          ),
        );
      ...
      data: (updatedPlayer) {
        return _StatefulDuaAudioPlayer(player: updatedPlayer);
      },
    );
  }
}

class _StatefulDuaAudioPlayer extends ConsumerStatefulWidget {
  const _StatefulDuaAudioPlayer({required this.player});

  final AudioPlayer player;

  @override
  ConsumerState<_StatefulDuaAudioPlayer> createState() =>
      _StatefulDuaAudioPlayerState();
}

class _StatefulDuaAudioPlayerState
    extends ConsumerState<_StatefulDuaAudioPlayer> {
